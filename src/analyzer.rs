//  .calc semantic analyzer implementation
extern crate nom;
...
//  ================= Types =================
pub type AnalyzedExpression = (AnalyzedTerm, Vec<(ExprOperator, AnalyzedTerm)>);
...
fn analyze_expr(
    ...
    parsed_expression: &ParsedExpr,
) -> Result<AnalyzedExpression, String> {
    let first_term = analyze_term(variables, &parsed_expression.0)?;
    let mut other_terms = Vec::<(ExprOperator, AnalyzedTerm)>::new();
    for term in &parsed_expression.1 {
    ...
    match parsed_factor {
        ...
        ParsedFactor::SubExpression(expr) => {
            Ok(AnalyzedFactor::SubExpression(
                Box::<AnalyzedExpression>::new(analyze_expr(variables, expr)?),
            ))
        }
    }
}
...
fn analyze_statement(
    ...
    match parsed_statement {
        ParsedStatement::Assignment(identifier, expr) => {
            ...
            let analyzed_expression = analyze_expr(variables, expr)?;
            Ok(AnalyzedStatement::Assignment(handle, analyzed_expression))
        }
        ...
        ParsedStatement::OutputOperation(expr) => {
            let analyzed_expression = analyze_expr(variables, expr)?;
            Ok(AnalyzedStatement::OutputOperation(analyzed_expression))
        }
    }
}
